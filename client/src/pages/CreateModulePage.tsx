...
export const CreateModulePage: React.FC = () => {
  ...
  const [isPrivateRepo, setIsPrivateRepo] = useState<boolean>(() => !!localStorage.getItem('moduleforge_github_token'));
  ...
  const handleValidateGithub = async (e: React.FormEvent) => {
    ...
    try {
      const tokenToUse = isPrivateRepo && githubToken.trim()
        ? githubToken.trim()
        : (localStorage.getItem('moduleforge_github_token') || undefined);

      if (githubToken.trim()) {
        localStorage.setItem('moduleforge_github_token', githubToken.trim());
      }

      const result = await validateGithubRepo(githubUrl, tokenToUse);
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto space-y-8">
      ...
      {activeTab === 'github' && (
        <div className="space-y-6">
          <form onSubmit={handleValidateGithub} className="p-8 rounded-3xl bg-white border border-[#E2E6E4] shadow-card space-y-5">
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-[#202524] font-mono flex items-center gap-2">
                ...
                <span>GitHub Repository URL (Public or Private)</span>
              </label>
              <input
                type="url"
                value={githubUrl}
            ...
            {/* Private Repo Toggle & Token Section */}
            <div className="p-4 rounded-2xl bg-[#F7F8F7] border border-[#E2E6E4] space-y-3">
              <div className="flex items-center justify-between">
                <label className="flex items-center gap-2.5 cursor-pointer text-xs font-bold text-[#202524]">
                  <input
                    type="checkbox"
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span>Private Repository (Authenticate with Token)</span>
                  </div>
                </label>
              ...
              {isPrivateRepo && (
                <div className="space-y-2 pt-1 animate-fade-in">
                  ...
                  <p className="text-[11px] text-[#6B7471] flex items-center gap-1.5">
                    <span>Requires <code>repo</code> read permissions.</span>
                    <a
                      href="https://github.com/settings/tokens?type=beta"
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-[#1F5E4B] hover:underline font-medium inline-flex items-center gap-0.5"
                    >
            ...
            <div className="flex items-center justify-between pt-2">
              <span className="text-xs text-[#6B7471]">
                {isPrivateRepo ? 'Streams private repository in-memory via secure GitHub API.' : 'Downloads repository archive from public branch.'}
              </span>
              <button
                type="submit"
